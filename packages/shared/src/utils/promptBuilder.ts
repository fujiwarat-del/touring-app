// ============================================================
// Touring App - Claude API Prompt Builder
// ============================================================
...
/**
 * Build the prompt string for Claude API route generation
 */
export function buildPrompt(req: GenerateRouteRequest): string {
  ...
  const returnStr =
    returnType === 'loop'
      ...
      ? '帰り: 行きとは異なる別ルートで出発地に戻る（往路と復路で異なる道を使うこと）'
      : '帰り: 目的地そのまま（帰還なし）';
...
${purposes.includes('林道') ? '10. 目的に「林道」が含まれているため、未舗装路・グラベル道・山岳林道・ダート道を積極的にルートに組み込むこと。舗装路よりも林道・砂利道・山道を優先し、オフロード走行の醍醐味が味わえるルートにすること。cautionには必ず路面状況（未舗装区間・ぬかるみ注意など）を記載すること' : ''}
${purposes.includes('キャンプ') ? '11. 目的に「キャンプ」が含まれているため、キャンプ場・オートキャンプ場・無料キャンプ場を経由地に積極的に含めること。焚き火・テント泊に適した自然豊かなエリアを優先し、キャンプ場名をwaypointObjectsのnameに明記すること' : ''}
${purposes.includes('湖・高原') ? '12. 目的に「湖・高原」が含まれているため、湖畔・高原・山岳リゾートを経由地に積極的に含めること。湖の展望スポット・高原の絶景道路を優先し、季節の景色（新緑・紅葉・雪景色など）も考慮すること' : ''}
${purposes.includes('城・史跡') ? '13. 目的に「城・史跡」が含まれているため、城・城址・神社仏閣・古戦場・史跡公園などを経由地に積極的に含めること。歴史的価値の高いスポットを優先し、waypointObjectsのdescriptionに簡単な歴史的説明を加えること' : ''}
`;
...
/**
 * Parse Claude API response text to extract JSON routes
 */
export function parseClaudeResponse(text: string): { routes: import('../types/index').Route[] } | null {
  try {

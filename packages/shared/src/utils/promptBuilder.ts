...
10. 【バイクアクセス厳守】以下の場所は絶対に経由地・目的地に含めないこと：
    ...
    - 【絶対禁止】フェリー・船・渡し船での移動が必要な経路は一切含めないこと。陸路のみで走破できるルートのみ提案すること
    ✅ 正しい例：道の駅・展望台（駐車場あり）・温泉施設・道路沿いの岬・湖畔の駐車場
    ❌ 悪い例：登山でしか行けない山頂、橋のない島、歩行者専用の遊歩道終点、フェリーが必要な離島
 ...
 */
/** スコアを安全に数値化（文字列・undefined・null 対応）*/
function toScore(v: unknown): number {
  const n = Number(v);
  if (isNaN(n) || n < 1) return 3; // デフォルト3（中間値）
  if (n > 5) return 5;
  return Math.round(n);
}

/** ルートデータのスコアフィールドを正規化 */
function normalizeRoutes(routes: any[]): import('../types/index').Route[] {
  return routes.map((r) => ({
    ...r,
    windingScore:    toScore(r.windingScore),
    sceneryScore:    toScore(r.sceneryScore),
    trafficScore:    toScore(r.trafficScore),
    difficultyScore: toScore(r.difficultyScore),
    waypointObjects:    r.waypointObjects    ?? [],
    highlightWaypoints: r.highlightWaypoints ?? [],
  }));
}
...
export function parseClaudeResponse(text: string): { routes: import('../types/index').Route[] } | null {
  try {
    ...
    return { routes: normalizeRoutes(parsed.routes) };
  } catch (e) {

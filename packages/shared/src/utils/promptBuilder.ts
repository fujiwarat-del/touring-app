...
export function buildPrompt(req: GenerateRouteRequest): string {
  ...
  const emptyRoadStr = emptyRoadMode
    ? '【エンプティロードモード有効: 高速道路・有料道路を完全回避し、できる限り交通量の少ない道を優先すること】'
    : '';
...
## 重要な注意事項
1. waypointObjectsには出発地・主要経由地・目的地を含め、5〜10地点を設定すること
2. highlightWaypointsには特に見どころとなる地点を2〜4か所設定すること
